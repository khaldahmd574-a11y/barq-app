...
# 🚫 قائمة الكلمات الممنوعة (يتم حظر أي رسالة تحتوي على أي منها)
...
RAW_KEYWORDS = [
    ...
    "للمطار", "لصبيا", "بسألكم", "السلام عليكم", "لمحليه", "هنا", "بالموسم", 
...
async def process_message(bot, message: Message):
    ...
    # 🚫 فحص الكلمات الممنوعة الأول: إذا وُجدت كلمة ممنوعة يتم استبعاد الرسالة فوراً
    for blocked_word in NORMALIZED_BLOCKED:
        if blocked_word in searchable_text:
            return

    # 🛑 فحص التكرارات لمنع إعادة إرسال النص نفسه
...
async def main():
    ...
    await userbot.start()
    await bot.start()
    print("✅ تم التشغيل والربط بنجاح (مع خاصية حظر الرسائل الممنوعة).")
