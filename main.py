...
# =========================
# القوائم الشاملة لطلبات العملاء
# =========================
...
SERVICE_WORDS = [
    "مندوب", "سواق", "سواقه", "سائق", "سائقه", "كابتن",
    "توصيل", "يوصلني", "يوصل لي", "يوصللي", "يوصلي", "يوصل", "توصيلة", "توصيله",
    "يجيب لي", "يجيبلي", "ياخذ لي", "ياخذلي", "يجيبني", "ياخذني",
    "مشوار", "مشاوير", "يروح", "ينقلني", "ينقل", "يرجعني", "يوديني", "ينزلني",
    "طلب", "طلبي", "غرض", "اغراض", "أغراض", "شحنة", "شحنه", "طرد", "كرتون"
]
...
DRIVER_PATTERNS = [
    ...
    "سيارة خاصة", "سياره خاصه", "حاضر للتوصيل", "جاهز للتوصيل"
]

LOCATION_WORDS = [
    # 1. المدن والمحافظات الرئيسية
    "جيزان", "جازان", "صبيا", "ضمد", "بيش", "الدرب", "ابو عريش", "أبو عريش",
    "العارضة", "العارضه", "صامطة", "صامطه", "الطوال", "فيفاء", "الداير", "الدائر",
    "احد المسارحه", "أحد المسارحة", "المسارحه", "المسارحة", "الحرث", "الموسم", "العيدابي",

    # 2. الإسكانات والأحياء والمخططات
    "اسكان الروان", "إسكان الروان", "الروان", "اسكان الخارش", "إسكان الخارش", "الخارش",
    "اسكان السهام", "اسكان الحصمة", "الحصمه", "الحصمة", "اسكان رمادة", "اسكان الشرفاء",
    "المطار", "الجامعة", "الجامعه", "السويس", "المجمع", "النخيل", "الاسكان", "الإسكان",
    "مخطط", "حي", "الشقيري", "الريان", "كربوس", "الكربوس", "البديع", "الخضراء", 
    "المضايا", "الخمس", "الدغارير", "محليه", "محلية", "البرج", "العسيله", "العسيلة",
    "الجهو", "اللقيه", "اللقية", "القمري", "القرعاوية", "الخبث", "بحره", "بحر ه",

    # 3. المستشفيات والمراكز الصحية
    "التخصصي", "المستشفى التخصصي", "مستشفى الحياة", "الحياة", "مستشفى صبيا",
    "مستشفى جازان", "المستشفى العام", "مستشفى ابو عريش", "مستشفى صامطة",
    "مستشفى العميس", "العميس", "مستشفى بوعريش", "العسكري", "المستشفى العسكري",

    # 4. المولات والمجمعات التجارية والوجهات العامة
    "الراشد", "الراشد مول", "كادي مول", "جراند مول", "صامطه مول", "الغيلان",
    "البيك", "المحطة", "المحطه", "سوق الاثنين", "سوق الخضار", "محطة النقل", "السوق الداخلي"
]
...
def is_customer_request(text):
    ...
    # 1. استبعاد منشورات السائقين فوراً
    if contains_phrase(norm_text, DRIVER_PATTERNS):
        return False

    # 2. قبول العبارات المباشرة السريعة (مثل: مشوار صامطة / توصيل ضمد)
    if any(k in norm_text for k in ["مشوار", "توصيل", "توصيله", "توصيلة"]):
        return True

    # 3. التأكد من وجود فعل تنقل أو نية طلب من القائمة
...
async def process_message(bot, message: Message):
    ...
    # بصمة الطلب لمنع التكرار عبر القروبات
    ...
    # إرسال الطلب للمشتركين المحددين
...
async def main():
    ...
    await userbot.start()
    await bot.start()
    print("✅ تم التشغيل بنجاح مع الفلترة الذكية المحدثة وربط القنوات والقروبات.")
