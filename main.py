...
# الكلمات المحدثة الشاملة بدون تكرار
RAW_KEYWORDS = [
    ...
    "مندوبه", "حتى", "ابغاه", "خاصه", "عندي", "بيطلع", "كنت", "ل للعارضه", "لدرب", 
    "ل للمطار", "لصبيا", "بسألكم", "السلام عليكم", "لمحليه", "هنا", "ب بالموسم", 
    "ب بصامطه", "بحتاج", "بيروحني", "بروح", "يجيني", "تجيني", "نوصل", "شباب", 
    "توصيل", "ذحين", "ل للكربوس", "ل لأبوعريش", "نمشي", "سعره", "كوفي", "روحه", 
    "ورجعه", "ينفعني", "مشاوير", "ل للدرب", "لين", "ل لأبها", "لابها", "يوصله", 
    "لبيش", "ف الشقيري", "يرجعني", "ب الضاحيه", "وجبه", "ل للبرج", "موجود", 
    "شعب", "الذيب", "وجاي", "ل للمجنه", "اشاره", "الشاشه", "ماشي", "يأخذ", 
    "الاهل", "يستلم", "سمسا", "ارامكس", "نفسها", "ب جيزان", "ب جازان", "ب ضمد", 
    "ب الشقيق", "طلبيه", "ابغاها", "معتمد", "يفتح", "المطاعم", "ب شارع", "يروح", 
    "داخل", "ضروري", "محطه", "معي", "معايه", "معانا", "الحين", "يقدر", "تقدر", 
    "يجيبها", "قطه", "قط", "الصوارمه", "هاف", "في", "حوض", "نخلان", "اولاد", 
...
async def process_message(bot, message: Message):
    ...
    # 2. منع التكرار المنسوخ (إذا نُشر نفس النص في قروب آخر)
    ...
    for original_word, norm_word in NORMALIZED_KEYWORDS.items():
        if norm_word in searchable_text:
            # إضافة بلمسة أمان للهاش للتأكد من عدم تكرار نفس النص
...
async def real_time_channel_and_group_scanner(userbot, bot):
    while True:
        try:
            # فحص أول 50 محادثة نشطة بنفس سرعة الكود الأول
...
async def main():
    ...
    await userbot.start()
    await bot.start()
    print("✅ تم التشغيل والربط بنجاح (فحص 50 محادثة كل 7 ثوانٍ بأمان تام وضبط منع التكرار).")
